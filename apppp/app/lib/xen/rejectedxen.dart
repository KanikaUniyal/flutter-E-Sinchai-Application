...
import 'package:app/xen/rejectedxendetail.dart';
import 'package:shared_preferences/shared_preferences.dart';
import 'pendingdetail.dart';
...
class Farmming {
  ...
  factory Farmming.fromJson(Map<String, dynamic> json) {
    return Farmming(
      id: json['id'] ?? "",
...
class rejectedMyAp extends StatefulWidget {
  @override
  _MyApppState createState() => _MyApppState();
}

class _MyApppState extends State<rejectedMyAp> {
  ...
  late List<Farmming> farmers;
  late List<Farmming> filteredFarmers = [];
  ...
  Future<List<Farmming>> fetchData(String status) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData['error']  ){
          ...
          if (data.isNotEmpty) {
            return data.map((json) => Farmming.fromJson(json)).toList();
          } else {
            Fluttertoast.showToast(
              msg: 'No data found.',
              toastLength: Toast.LENGTH_SHORT,
                                      ...
                                      child: ElevatedButton(
                                        onPressed: () {
                                          Navigator.push(
                                            ...
                                            MaterialPageRoute(
                                              builder: (context) =>
                                                  rejecteddetailspage(farmer: farmer),
                                            ),
                                          );
                                        },
