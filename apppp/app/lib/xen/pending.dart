...
class PendingMyAp extends StatefulWidget {
  @override
  _MyApppState createState() => _MyApppState();
}

class _MyApppState extends State<PendingMyAp> {
 ...
 // final String sessionKey =
  ...
  late List<Farmers> farmers;
  late List<Farmers> filteredFarmers = [];
  ...
  Future<List<Farmers>> fetchData(String status) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonData['error'] &&
        ...
        if (data.isNotEmpty) {
          return data.map((json) => Farmers.fromJson(json)).toList();
        }
        else {
