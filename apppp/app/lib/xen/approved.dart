...
class Farmerss {
  ...
  factory Farmerss.fromJson(Map<String, dynamic> json) {
    return Farmerss(
      id: json['id'] ?? "",
...
class _MyApppState extends State<My> {

  final String apiUrl = ApiUrls.applicationList;

  late String sessionKey;
  late List<Farmerss> farmers;
  late List<Farmerss> filteredFarmers = [];
  ...
  // Future<List<Farmerss>> fetchData(String status) async {
  //   final response = await http.post(
  //     Uri.parse(apiUrl),
  //     body: {"session_key": sessionKey, "status": status},
  //   );
  //
  //   if (response.statusCode == 200) {
  ...
  //
  //     if (data.isNotEmpty) {
  //      return data.map((json) => Farmerss.fromJson(json)).toList();
  //     } else {
                                ...
                                child: ElevatedButton(
                                  onPressed: () {
                                    Navigator.push(
                                      ...
                                      MaterialPageRoute(
                                        builder: (context) =>
                                            approdetailspage(farmer: farmer),
                                      ),
                                    );
                                  },
