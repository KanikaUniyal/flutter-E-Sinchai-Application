...
class BlinkingStatusButton extends StatefulWidget {
  ...
  const BlinkingStatusButton({Key? key, required this.status}) : super(key: key);
  @override
    ...
    _timer = Timer.periodic(Duration(milliseconds: 500), (timer) {
      setState(() {
        ...
        switch (status) {
          case 'pending':
          case 'in process':
            buttonColor = Colors.orange; // Change to orange for pending/in process
            break;
          case 'approved':
          ...
          case 'rejected':
            ...
            break;
          default:
            buttonColor = Colors.grey; // Default color for unknown status
        }
        isVisible = !isVisible;
      });
    });
            ...
            child: Visibility(
              ...
              child: ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(
                  ...
                  padding: EdgeInsets.all(0), backgroundColor: buttonColor,
                ),
...
class DetailPage extends StatefulWidget {
  final Farmerr farmer;

  const DetailPage({Key? key, required this.farmer}) : super(key: key);
...
class _DetailPageState extends State<DetailPage> {
  ...
  Widget build(BuildContext context) {
    ...
    String requestApproveStatus = applicationTransactions != null && applicationTransactions.isNotEmpty
        ...
        : 'Unknown'; // Provide a default value or handle null as needed
        ...
        child: Column(
          ...
          Container(
            ...
            child: SingleChildScrollView(
                      ...
                      children: [
                        if (applicationTransactions != null)
                          for (var i = 0; i < applicationTransactions.length; i++)
                            if (applicationTransactions[i] != null)

              Row(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
              // Left side: BlinkingStatusButton
              SizedBox(
              width: 60,
             ...
             // child:   BlinkingStatusButton(status: requestApproveStatus),
              // BlinkingStatusButton(
              //   status: applicationTransactions[i]['request_approve_status'] ?? '', // Pass the status to determine the button color
              // ),
            ),
            //SizedBox(width: 2), // Add spacing between the left and right sides
            // Right side: Application transaction data
            Expanded(
                            child:Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
    ...
    return Column(
      children: [
        AnimatedOpacity(
          ...
          child: buildStatusButton('', status, ''),
        ),
      ],
    );
  }
