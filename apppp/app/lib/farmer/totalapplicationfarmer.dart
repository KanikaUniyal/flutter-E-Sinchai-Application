...
import 'package:app/farmer/totalapplicationfarmerdetail.dart';
...
class Farmerr {
  ...
  factory Farmerr.fromJson(Map<String, dynamic> json) {
    ...
    return Farmerr(
      ...
      outlet: json['outlet'] ?? "",
      // patwari_name: farmerJson != null ? farmerJson['patwari_name'] ?? "" : "",
...
class _MyAppState extends State<MyAp>  {
  ...
  late List<Farmerr> farmers;
  late List<Farmerr> filteredFarmers = [];
  ...
  Future<List<Farmerr>> fetchData(String status) async {
    try {
      ...
      if (response.statusCode == 200) {
       // final List<dynamic> data = json.decode(response.body)
        ...
        if (data.isNotEmpty) {
          return data.map((json) => Farmerr.fromJson(json)).toList();
        }
      //   else {
                                  ...
                                  Container(
                                    ...
                                    child: ElevatedButton(
                                      onPressed: () {
                                        Navigator.push(
                                          context,
                                          MaterialPageRoute(builder: (context) => DetailPage(farmer: farmer)),
                                        );
                                        // Handle button tap
                                      },
                                      style:
